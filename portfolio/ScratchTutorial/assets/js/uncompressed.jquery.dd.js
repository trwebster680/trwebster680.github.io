...
;(function($) { 
   ...
   var msOldDiv = ""; 
   var dd = function(element, options, aDefaultTitleImgPath)
   {
		//CP specific function
		var cpDefaultTitleImageSrc = aDefaultTitleImgPath;
	...
	var createA = function(currentOptOption, current, currentopt, tp) {
		...
		a_array[aid] = {html:arrow, value:sValue, text:sText, index:currentOptOption.index, id:aid};
		var innerStyle = getOptionsProperties(currentOptOption);
		if(matchIndex(currentOptOption.index)==true) {
		 aTag += '<a tabindex=-1 href="javascript:void(0);" class="'+styles.selected+' '+sEnabledClass+clsName+'"';
		} else {
		aTag += '<a tabindex=-1 href="javascript:void(0);" class="'+sEnabledClass+clsName+'"';
		};
	...
	var createTitleDiv = function () {
		...
		var sText = "";
		var arrow = cpDefaultTitleImageSrc;
		...
		sDiv += '>';
		sDiv += '<span id="'+arrowid+'" class="'+styles.arrow+'"></span><span class="'+styles.ddTitleText+'" id="'+titletextid+'">'+arrow + '</span></div>';
		return sDiv;
	};
	
	var resetTitleDiv = function() {	
		var titleid = getPostID("postTitleID");
		var arrowid = getPostID("postArrowID");
		var titletextid = getPostID("postTitleTextID");
		var inputhidden = getPostID("postInputhidden");
		...
		var lTitleInnerHTML = arrow;// + "<span class='"+styles.ddTitleText+"'>"+sText+"</span>";
		
		var lElem = document.getElementById(titletextid);
		if(lElem)
			lElem.innerHTML = lTitleInnerHTML;			
	};
	...
	var createDropDown = function () {
		...
		$("#"+id).css("height", $("#"+elementid).height() + "px");
		$("#"+id).find("."+styles.ddTitle).css("height", $("#"+elementid).height() + "px");	
		//$("#"+id).find("."+styles.ddTitleText).css("height", $("#"+elementid).height() + "px");	
	...
	var shouldOpenOpposite = function() {
		var childid = getPostID("postChildID");
		var main = document.getElementById(getPostID("postID")).parentElement.id;
		var pos = $("#"+main).position();
		var mH = $("#"+main).height();
		var wH = $("#div_Slide").height();
	...
	this.open = function() {
		...
		if($("#"+childid).css("display")=="none") {
			if(a_array[$("#"+childid +" a.selected").prop("id")])
	...
	$.fn.extend({
	        msDropDown: function(options, aDefaultTitleImgPath)
	        {				
	            return this.each(function()
	            {
	               //if ($(this).data('dd')) return; // need to comment when using refresh method - will remove in next version
	               var mydropdown = new dd(this, options, aDefaultTitleImgPath);
